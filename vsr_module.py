...
mp_options   = vision.FaceLandmarkerOptions(base_options=base_options, num_faces=1)
...
def extract_lip_frames(frames_bgr: list) -> list:
    ...
    for frame in frames_bgr:
        ...
        if result.face_landmarks:
            ...
            x1 = max(min(lip_x) - 30, 0);  x2 = min(max(lip_x) + 30, w)
            y1 = max(min(lip_y) - 40, 0);  y2 = min(max(lip_y) + 40, h)
...
def transcribe_lips(frames_bgr: list) -> dict:
    """Send lip frames to Gemini, return text + confidence."""
    ...
    try:
        ...
        return {"text": text, "confidence": 0.55}   # VSR is always lower confidence

    except Exception as e:
        err = str(e)
        if "429" in err:
            return {"text": "", "confidence": 0.0, "error": "rate_limit"}
        return {"text": "", "confidence": 0.0, "error": err}